...
export default function Avatar({children, src, alt, className, ...rest}) {
    const colors = ['navy', 'pink', 'red', 'blue', 'green']
    const randomIndex = Math.floor(Math.random() * colors.length)
    const randomColor = colors[randomIndex]
    const allClassNames = classnames(className, randomColor)
    const image = src ? <img src={src} alt={alt} /> : children ? children : <IoPersonSharp />
...
/* Instructor solution * Instructor did not pass classes as props *
import React from "react"
import { IoPersonSharp } from "react-icons/io5"
